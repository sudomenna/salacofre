 ...
 * logs only the 404 warn case because 404 is a recoverable no-op that
 * callers should not need to handle specially.
 */

import { type EA20, EA20Schema } from "./ea20-schema";
 ...
 *
 * RF-010 + Res. 23.736/2024 (referência de práticas) exige que sistemas
 * cadastrados como "interessados na divulgação" se identifiquem com um
 * User-Agent reconhecível. O texto exato depende da resolução TSE 2026 — este
 * placeholder (decisão D-2 em tasks.md) será revisado no T22 quando a
 * resolução 2026 for publicada.
 */
const USER_AGENT = "SalaCofre/1.0 (interessado-divulgacao-cadastrado)";
...
export async function fetchEA20(opts: {
  ...
  if (res.status === 404) {
    ...
    // several minutes after polls close. Log at warn so ops can track how
    // many zones are still pending without it being noise-level.
    logWarn("TSE 404 — zona sem dados", { url: opts.url });
  ...
  if (!res.ok) {
    ...
    throw new TSEError(res.status, opts.url, bodySample);
  }
  ...
  // Validate against the EA20 Zod schema (strict at top level).
  // ZodError thrown here means the TSE changed the EA20 format — treated as a
  // parse failure so the retry wrapper does NOT retry (parse errors are
