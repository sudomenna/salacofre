...
|---|---|---|---|
| **`zona` (~2.651 zonas reais na tabela `zonas`)** | ~2.651 | **5.302 (medido)** | `TSE_GRANULARIDADE=zona` |
| `uf` | 27 + 1 BR (só Presidente) | 27×2 + 1 = **55** | `TSE_GRANULARIDADE=uf` — **quebrado no modelo em produção, ver aviso acima**; mantido só como fallback de emergência (telas nacional/UF sozinhas, sem modelo funcional) |

Só Presidente (cargo 0001) tem arquivo agregado de Brasil; Governador (0003) tem UF/Município/Zona apenas — daí o "+1" e não "+2". `buildProductionTargetsZona` (`lib/tse/targets.ts:688`) lê a tabela `zonas` do Neon — 2.651 linhas confirmadas em 05/09, não uma estimativa.
...
| `TSE_GRANULARIDADE` | `zona` (desde 05/09, E4) | `lib/tse/targets.ts:399` (`getGranularidade`) | `uf` \| `zona`. `uf` = 27 UFs × cargos + 1 arquivo BR de Presidente ≈ **55 GETs/ciclo**, mas **está quebrado no modelo em produção** (ver aviso no topo de [dimensionamento do fan-out](#tse--dimensionamento-do-fan-out-revisado-em-2026-09-05) — sentinela `cod_zona=0` sem peso em `eleitorado`). `zona` = ~2.651 zonas × cargos ≈ **5.302 GETs/ciclo (medido)**, necessária para o swing do modelo (RF-011/012) e o único modo funcional hoje. Valor inválido cai em `uf` com warn. **`getGranularidade` continua com default de código `uf`** — mudar esse default para `zona` é uma decisão de código pendente de confirmação (fora do escopo desta tarefa; ver relatório do ensaio de escala de 05/09), **defina `TSE_GRANULARIDADE=zona` explicitamente** até lá. |
